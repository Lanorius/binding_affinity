...
trainloader_1 = torch.utils.data.DataLoader(dataset=(train2+train3+train4+train5), batch_size=batch_size, shuffle=False)
trainloader_2 = torch.utils.data.DataLoader(dataset=(train1+train3+train4+train5), batch_size=batch_size, shuffle=False)
trainloader_3 = torch.utils.data.DataLoader(dataset=(train1+train2+train4+train5), batch_size=batch_size, shuffle=False)
trainloader_4 = torch.utils.data.DataLoader(dataset=(train1+train2+train3+train5), batch_size=batch_size, shuffle=False)
trainloader_5 = torch.utils.data.DataLoader(dataset=(train1+train2+train3+train4), batch_size=batch_size, shuffle=False)
...
trainloader = torch.utils.data.DataLoader(dataset=train_main, batch_size=batch_size, shuffle=False)
testloader = torch.utils.data.DataLoader(dataset=test, batch_size=batch_size, shuffle=False)
...
def trainer(data_for_training, amount_of_epochs, optimi, tuning=True):
    all_losses = []
        ...
        for i, (prot_comps, label) in enumerate(data_for_training):
            ...
            if not tuning:
                if i % 40 == 39:  # print every 40 mini-batches
                    print('[%d, %5d] loss: %.7f' % (epoch_index + 1, i + 1, running_loss / 40))
                    all_losses += [running_loss/40]
                    running_loss = 0.0
...
for test_train_index in tqdm(range(5)):
    for optimization in tqdm(range(number_of_random_draws)):
        ...
        trainer(training_sets[test_train_index], number_of_epochs, optimizer)
        performance = tester(testing_tests[test_train_index])
        if performance > current_best_r2m:
            current_best_r2m = performance
            best_parameters_overall = [learning_rate, number_of_epochs]
...
optimizer = optim.Adam(model.parameters(), lr=best_parameters_overall[0])

trainer(trainloader, best_parameters_overall[1], optimizer, tuning=False)
